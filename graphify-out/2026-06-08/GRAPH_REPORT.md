...
## Corpus Check
- 89 files · ~32,121 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1126 nodes · 2420 edges · 82 communities (66 shown, 16 thin omitted)
- Extraction: 78% EXTRACTED · 22% INFERRED · 0% AMBIGUOUS · INFERRED: 528 edges (avg confidence: 0.52)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `105ccb20`
...
3. `PipelineConfig` - 57 edges
4. `Neo4jStore` - 57 edges
...
Cohesion: 0.06
Nodes (53): LanguageSlicer, Node, Parser, Symbol-level context slicing for language-agnostic code extraction., LanguageSlicer, Language-agnostic slicing Protocol.  Defines the shared contract that every lang, Extract and stitch back a named symbol within a source file., Return a SymbolSlice for the named function/class, or None if not found. (+45 more)

### Community 2 - "Community 2"
Cohesion: 0.17
Nodes (23): OllamaClient, Neo4jStore, Async Neo4j driver wrapper for Function nodes and SIMILAR_TO edges., Async Neo4j driver wrapper for Function nodes and SIMILAR_TO edges., PipelineConfig, QueryMatch, QueryResult, Semantic search engine for functions stored in Neo4j.  Embeds a free-text query (+15 more)

### Community 3 - "Test Infrastructure & Fixtures"
Cohesion: 0.16
Nodes (18): load_pipeline_config(), Pipeline configuration loader.  Reads the top-level ``neo4j`` block and the per-, Load pipeline configuration for a repository.      Args:         config_path: Pa, Load pipeline configuration for a repository.      Args:         config_path: Pa, Load pipeline configuration for a repository.      Args:         config_path: Pa, Load pipeline configuration for a repository.      Args:         config_path: Pa, _require_env(), _validate_timezone() (+10 more)
...
Cohesion: 0.10
Nodes (37): get_system_context_path(), GraphConfig, Return the expanded absolute path for the system-level context store.      Resol, Return the expanded absolute path for the system-level context store.      Resol, Graph lifecycle settings for a repository.      Attributes:         mode: Storag, Graph lifecycle settings for a repository.      Attributes:         mode: Storag, GraphHandle, Immutable reference to a resolved, validated knowledge graph.      Produced excl (+29 more)

### Community 10 - "Community 10"
Cohesion: 0.11
Nodes (26): Build and return a bounded ContextPackage.          Must be deterministic and mu, build_repository_context_payload(), _dedupe_preserve_order(), DependencySummaryItem, _normalize_path(), Versioned contract for retrieval -> coder repository context payloads.  This mod, Validate structural invariants for a repository context payload., Build a versioned, deterministic context payload from ContextPackage. (+18 more)

### Community 12 - "Community 12"
Cohesion: 0.19
Nodes (9): DescriptionService, Description service for LLM-generated function summaries.  Wraps ``OllamaClient., Remove triple-backtick code fences that models emit despite instructions., Generates structured JSON descriptions of functions via OllamaClient., Populate ``record.description`` and ``record.description_status`` in-place., Generate descriptions for all records in-place, respecting concurrency limit., _strip_fences(), FunctionRecord (+1 more)

### Community 19 - "Community 19"
Cohesion: 0.12
Nodes (21): bool, diff_generator_node(), Compute a unified diff between the original and generated code.      Expected st, Aggregate export surface for graph nodes., Static validator node.  Replaces the former reviewer node. Responsibilities are, Validate generated code for structural correctness.      Runs Python syntax vali, static_validator_node(), Shared helpers for graph node implementations. (+13 more)
...
Cohesion: 0.11
Nodes (23): branch_creator_node(), Branch creator node.  Creates (or checks out) a task branch in the target reposi, Create a task branch in the target repository.      Reads ``repo_path`` and ``ta, Git committer node.  Stages the modified target file and creates a git commit on, Semantic validator node.  Evaluates whether the generated code correctly satisfi, emit_event(), emit_failure(), emit_success() (+15 more)
...
Cohesion: 0.15
Nodes (16): datetime, _build_semantic_feedback(), Format the LLM evaluation into a concise string for the coder prompt., Format the LLM evaluation into a concise string for the coder prompt., Evaluate task-intent alignment of the generated code using an LLM judge.      Re, Evaluate task-intent alignment and regression risk of the generated change., semantic_validator_node(), new() (+8 more)
...
Cohesion: 0.29
Nodes (9): AppConfig, get_coder_model(), get_coder_model_config(), get_ollama_base_url(), get_primary_model(), Centralized configuration loader for the project.  Configuration is loaded from, Return the full ModelConfig for the coder role., Return the full ModelConfig for the coder role. (+1 more)
...
Cohesion: 0.22
Nodes (15): compute_similarity_edges(), Cosine similarity graph builder using Neo4j HNSW vector indexes.  For each funct, Compute SIMILAR_TO edges by querying Neo4j vector indexes.      Args:         st, A SIMILAR_TO relationship between two Function nodes., A SIMILAR_TO relationship between two Function nodes., A SIMILAR_TO relationship between two Function nodes., A SIMILAR_TO relationship between two Function nodes., SimilarityConfig (+7 more)

### Community 41 - "Community 41"
Cohesion: 0.11
Nodes (15): Return ``{function_id: source_hash}`` for all live functions in the repo., Return ``{function_id: source_hash}`` for all live functions in the repo., Return ``[(id, code_embedding, description_embedding)]`` for all live functions., Return ``[(id, code_embedding, description_embedding)]`` for all live functions., Return top-N code-similar functions using the HNSW vector index.          Result, Return top-N code-similar functions using the HNSW vector index.          Result, Return top-N description-similar functions using the HNSW vector index., Return top-N description-similar functions using the HNSW vector index. (+7 more)
...
Cohesion: 0.08
Nodes (32): _build_pipeline_blocks(), _build_report_blocks(), notify_pipeline_result(), notify_report_result(), notify_scheduled_run(), Fire-and-forget Slack notification for pipeline completion events., Build a Slack Block Kit block list from a PipelineResult., Post a pipeline completion or failure notice to the configured Slack channel. (+24 more)

### Community 49 - "Community 49"
Cohesion: 0.15
Nodes (14): GraphState: original_code field, _build_context_slice(), file_reader_node(), Read the target file (or select one) and return its contents.      Expected stat, Read the target file and, when a target symbol is set, build a context slice., Assemble the context dict for the coder's focused prompt., Pick the first Python file in a repo root deterministically., Pick the first Python file in a repo root deterministically. (+6 more)

### Community 50 - "Community 50"
Cohesion: 0.33
Nodes (6): get_retrieval_config(), Return retrieval limits and behavior for the repository matching ``repo_path``., Return retrieval limits and behavior for the repository matching ``repo_path``., Controls how many files and tokens the retrieval pipeline may assemble.      Att, Controls how many files and tokens the retrieval pipeline may assemble.      Att, RetrievalConfig

### Community 51 - "Community 51"
Cohesion: 0.33
Nodes (6): get_planner_config(), PlannerConfig, Return planner settings for the repository matching ``repo_path``., Return planner settings for the repository matching ``repo_path``., Controls how many files the planner node may select for modification.      Attri, Controls how many files the planner node may select for modification.      Attri

### Community 52 - "Community 52"
Cohesion: 0.07
Nodes (48): _extract_from_file(), _is_test_file(), Function extractor.  Uses tree-sitter directly to walk the AST of each source fi, Return one FunctionRecord per function/method found in the repository., Return one FunctionRecord per function/method found in the repository., Return True if the relative path matches any configured test pattern., Return True if the relative path matches any configured test pattern., _record_id() (+40 more)

### Community 54 - "Community 54"
Cohesion: 0.31
Nodes (3): ExecutionLoop, Background execution loop that consumes tasks from a `TaskQueue`.      Responsib, Task
...
Cohesion: 0.18
Nodes (12): FunctionExtractor, Extracts every function and method from a repository as FunctionRecords.      Em, Extracts every function and method from a repository as FunctionRecords.      Em, EmbeddingPipeline, Main pipeline orchestrator.  Runs the full extraction → embedding → description, Orchestrates all pipeline stages for a single repository., Execute all pipeline stages and return a summary., Execute all pipeline stages and return a summary. (+4 more)

### Community 58 - "Community 58"
Cohesion: 0.13
Nodes (11): _ddl(), Neo4j data store for Function nodes and SIMILAR_TO relationships.  Uses the offi, Close the driver connection pool., Close the driver connection pool., Idempotently create constraints and indexes.          Args:             vector_d, Format a DDL template with integer-only substitutions and assert LiteralString., Mark functions not in ``seen_ids`` as deleted. Returns count., Mark functions not in ``seen_ids`` as deleted. Returns count. (+3 more)

### Community 59 - "Community 59"
Cohesion: 0.40
Nodes (5): git_committer_node(), Stage and commit the modified target file.      Expected state keys:     - ``, Stage and commit the modified target file.      Expected state keys:     - ``rep, GraphState, RunContext
...
Cohesion: 0.22
Nodes (6): CronTrigger, Fires a PipelineTask on a cron schedule.      Args:         cron_expr: Standard, Start the background scheduling loop. Idempotent., Cancel the scheduling loop and wait for it to exit. Idempotent., str, TaskQueue

### Community 62 - "Community 62"
Cohesion: 0.11
Nodes (24): Markdown section renderers for the pipeline report.  Each function accepts pre-c, Section 3 — embedding and description coverage tables., Section 2 — delta since previous run.      Returns the markdown lines and the de, Section 2 — delta since previous run.      Returns the markdown lines and the de, Section 3 — embedding and description coverage tables., Section 3 — embedding and description coverage tables., Section 1 — report header and metadata table., Section 6 — top N most similar function pairs. (+16 more)
...
Cohesion: 0.31
Nodes (7): FIFO Task Queue - In-memory Deterministic Ordering, Cron-based pipeline trigger.  Reads a cron expression and enqueues a PipelineTas, TaskQueue, Task types for the scheduler.  Two concrete work units and their union — the typ, Scheduler work unit wrapping a validated TaskRequest., Task, Task

### Community 68 - "Community 68"
Cohesion: 0.15
Nodes (13): ReporterConfig, Section 5 — similarity score distribution across four bins., Section 5 — similarity score distribution across four bins., Section 5 — similarity score distribution across four bins., Section 10 — file cohesion scores (ascending, most fragmented first)., Section 12 — duplication clusters at the configured similarity threshold., Section 10 — file cohesion scores (ascending, most fragmented first)., Section 10 — file cohesion scores (ascending, most fragmented first). (+5 more)

### Community 69 - "Community 69"
Cohesion: 0.21
Nodes (12): _combined_sim(), _compute_clusters(), _compute_cohesion_scores(), _cosine(), Pure Python analysis functions for the pipeline report.  These helpers operate o, Combined similarity matching the weighting used by similarity.py., Compute average pairwise similarity for each group (file or class).      Args:, Build connected components from similarity edges via BFS.      Each node is iden (+4 more)

### Community 70 - "Community 70"
Cohesion: 0.17
Nodes (11): LimitsConfig, _now_iso(), Data contracts for the function embedding pipeline.  All dataclasses here are pl, Source text truncation limits and embedding context window., Source text truncation limits and embedding context window., Source text truncation limits and embedding context window., Source text truncation limits and embedding context window., Source text truncation limits and embedding context window. (+3 more)

### Community 71 - "Community 71"
Cohesion: 0.17
Nodes (11): _find_previous_report(), Return the parsed JSON of the most recent prior report, or None., Section 7 — top N most connected functions with intra/inter breakdown., Section 7 — top N most connected functions with intra/inter breakdown., Section 7 — top N most connected functions with intra/inter breakdown., Section 11 — class cohesion scores (omitted when no classes present)., Section 11 — class cohesion scores (omitted when no classes present)., render_class_cohesion() (+3 more)
...
Cohesion: 0.22
Nodes (8): WorkflowExecutor as Single Orchestration Boundary, Executor that runs a workflow graph for a given `Task`.      The executor acce, Executor that runs a workflow graph for a given `Task`.      The executor accept, WorkflowExecutor, TaskQueue, TaskDispatcher, TaskQueue, WorkflowExecutor

### Community 77 - "Community 77"
Cohesion: 0.20
Nodes (10): Section 4 — graph overview statistics, language breakdown, and isolated function, Section 4 — graph overview statistics, language breakdown, and isolated function, Section 4 — graph overview statistics, language breakdown, and isolated function, Section 13 — heuristic flags raised by the analysis., Section 13 — heuristic flags raised by the analysis., Section 13 — heuristic flags raised by the analysis., render_graph_overview(), render_heuristic_flags() (+2 more)
...
Cohesion: 0.29
Nodes (5): Create vector indexes once the embedding dimension is known., Create vector indexes once the embedding dimension is known., Batch-upsert Function nodes using UNWIND for efficiency., Batch-upsert Function nodes using UNWIND for efficiency., FunctionRecord

### Community 80 - "Community 80"
Cohesion: 0.33
Nodes (6): get_semantic_model(), get_semantic_model_config(), Return the full ModelConfig for the semantic validator role.      Prefers models, Return the full ModelConfig for the semantic validator role.      Prefers models, Return the model name for the semantic validator.      Prefers models["semantic_, Return the model name for the semantic validator.      Prefers models["semantic_

### Community 81 - "Community 81"
Cohesion: 0.50
Nodes (4): Section 9 — top N files by function count, flagging god files., Section 9 — top N files by function count, flagging god files., Section 9 — top N files by function count, flagging god files., render_files_by_function_count()
...
- **Why does `GraphState` connect `Community 45` to `Community 33`, `Community 66`, `Community 39`, `Community 9`, `Community 10`, `Community 74`, `Community 28`, `Community 49`, `Community 19`, `Community 26`, `Community 59`, `Community 60`, `Community 29`, `Community 30`, `Community 31`?**
  _High betweenness centrality (0.272) - this node is a cross-community bridge._
- **Why does `OllamaClient` connect `Community 2` to `Community 32`, `Community 4`, `LLM Pipeline & Sandbox`, `Community 73`, `Community 12`, `Community 46`, `Community 19`, `Community 57`?**
  _High betweenness centrality (0.244) - this node is a cross-community bridge._
- **Why does `datetime` connect `Community 30` to `Community 70`, `Community 71`, `Community 60`, `Community 48`, `Community 57`, `Community 28`, `Community 31`?**
