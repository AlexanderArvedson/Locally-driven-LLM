# Graph Report - Locally-driven-langgraph-LLM  (2026-06-07)

## Corpus Check
- 79 files · ~30,999 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1126 nodes · 2394 edges · 74 communities (59 shown, 15 thin omitted)
- Extraction: 77% EXTRACTED · 23% INFERRED · 0% AMBIGUOUS · INFERRED: 549 edges (avg confidence: 0.52)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `e7316a77`
...
3. `Neo4jStore` - 63 edges
4. `PipelineConfig` - 62 edges
...
Cohesion: 0.09
Nodes (32): load_pipeline_config(), Pipeline configuration loader.  Reads the top-level ``neo4j`` block and the per-, Load pipeline configuration for a repository.      Args:         config_path: Pa, Load pipeline configuration for a repository.      Args:         config_path: Pa, Load pipeline configuration for a repository.      Args:         config_path: Pa, Load pipeline configuration for a repository.      Args:         config_path: Pa, _require_env(), _validate_timezone() (+24 more)
...
Cohesion: 0.16
Nodes (17): DescriptionService, Generates structured JSON descriptions of functions via OllamaClient., Generates structured JSON descriptions of functions via OllamaClient., FunctionExtractor, Extracts every function and method from a repository as FunctionRecords.      Em, Extracts every function and method from a repository as FunctionRecords.      Em, Extracts every function and method from a repository as FunctionRecords.      Em, Extracts every function and method from a repository as FunctionRecords.      Em (+9 more)
...
Cohesion: 0.10
Nodes (25): bool, file_writer_node(), Write generated content to disk or apply a unified diff.      Expected state inp, Write generated content to disk or apply a unified diff.      Expected state inp, Static validator node.  Replaces the former reviewer node. Responsibilities are, Validate generated code for structural correctness.      Runs Python syntax vali, static_validator_node(), Shared helpers for graph node implementations. (+17 more)
...
Cohesion: 0.14
Nodes (19): branch_creator_node(), Branch creator node.  Creates (or checks out) a task branch in the target reposi, Create a task branch in the target repository.      Reads ``repo_path`` and ``ta, Git committer node.  Stages the modified target file and creates a git commit on, Graphify indexer — internal graph-building utility.  Provides `build_ast_graph`,, emit_event(), emit_failure(), emit_success() (+11 more)
...
Cohesion: 0.06
Nodes (32): LiteralString, _ddl(), Neo4j data store for Function nodes and SIMILAR_TO relationships.  Uses the offi, Close the driver connection pool., Idempotently create constraints and indexes.          Args:             vector_d, Close the driver connection pool., Close the driver connection pool., Async Neo4j driver wrapper for Function nodes and SIMILAR_TO edges. (+24 more)
...
Cohesion: 0.16
Nodes (21): ndarray, A SIMILAR_TO relationship between two Function nodes., A SIMILAR_TO relationship between two Function nodes., A SIMILAR_TO relationship between two Function nodes., A SIMILAR_TO relationship between two Function nodes., SimilarityConfig, SimilarityEdge, compute_similarity_edges() (+13 more)
...
Cohesion: 0.11
Nodes (22): _build_pipeline_blocks(), _build_report_blocks(), notify_pipeline_result(), notify_report_result(), notify_scheduled_run(), Fire-and-forget Slack notification for pipeline completion events., Build a Slack Block Kit block list from a PipelineResult., Post a pipeline completion or failure notice to the configured Slack channel. (+14 more)
...
Cohesion: 0.16
Nodes (11): FunctionRecord, One extracted function or method from a source file., Embedding service for code and description vectors.  Wraps ``OllamaClient.embed(, OllamaClient, PipelineConfig, bool, FunctionRecord, int (+3 more)
...
Cohesion: 0.13
Nodes (15): EmbeddingService, Embed descriptions for all records in-place, respecting concurrency limit., Embed descriptions for all records in-place, respecting concurrency limit., Embed source code for all records in-place, respecting concurrency limit., Embed descriptions for all records in-place, respecting concurrency limit., Generates code and description embeddings using OllamaClient., Generates code and description embeddings using OllamaClient., Populate ``record.code_embedding`` in-place.          Logs and skips on failure (+7 more)

### Community 58 - "Community 58"
Cohesion: 0.15
Nodes (11): Create vector indexes once the embedding dimension is known., Return ``{function_id: source_hash}`` for all live functions in the repo., Return ``{function_id: source_hash}`` for all live functions in the repo., Return ``{function_id: source_hash}`` for all live functions in the repo., Return ``{function_id: source_hash}`` for all live functions in the repo., Return ``{function_id: source_hash}`` for all live functions in the repo., Mark functions not in ``seen_ids`` as deleted. Returns count., Mark functions not in ``seen_ids`` as deleted. Returns count. (+3 more)
...
Cohesion: 0.08
Nodes (42): Language, _extract_from_file(), _find_functions(), _get_class_name(), _get_identifier(), _get_name_token(), _get_property_name(), _get_ts_language() (+34 more)

### Community 68 - "Community 68"
Cohesion: 0.20
Nodes (10): PipelineResult, Summary of a completed pipeline run., Summary of a completed pipeline run., Summary of a completed pipeline run., Summary of a completed pipeline run., Summary of a completed pipeline run., Summary of a completed pipeline run., Summary of a completed pipeline run. (+2 more)
...
Cohesion: 0.09
Nodes (22): Return ``[(id, code_embedding, description_embedding)]`` for all live functions., Return ``[(id, code_embedding, description_embedding)]`` for all live functions., Return ``[(id, code_embedding, description_embedding)]`` for all live functions., Delete all SIMILAR_TO edges originating from functions in this repo.          Ca, Delete all SIMILAR_TO edges originating from functions in this repo.          Ca, Return ``[(id, code_embedding, description_embedding)]`` for all live functions., Return ``[(id, code_embedding, description_embedding)]`` for all live functions., Return ``[(id, code_embedding, description_embedding)]`` for all live functions. (+14 more)

### Community 77 - "Community 77"
Cohesion: 0.40
Nodes (5): _compute_clusters(), Build connected components from similarity edges via BFS.      Each node is iden, Build connected components from similarity edges via BFS.      Each node is iden, Build connected components from similarity edges via BFS.      Each node is iden, Build connected components from similarity edges via BFS.      Each node is iden
...
- **Why does `GraphState` connect `Community 30` to `Community 33`, `Community 39`, `Community 9`, `Community 59`, `Community 12`, `Community 45`, `Community 46`, `Community 61`, `Community 19`, `Community 26`, `Community 27`, `Community 28`, `Community 29`?**
  _High betweenness centrality (0.256) - this node is a cross-community bridge._
- **Why does `OllamaClient` connect `Community 2` to `Community 32`, `Community 4`, `LLM Pipeline & Sandbox`, `Community 40`, `Community 10`, `Community 49`, `Community 19`, `Community 52`, `Community 57`?**
  _High betweenness centrality (0.250) - this node is a cross-community bridge._
- **Why does `datetime` connect `Test Infrastructure & Fixtures` to `Community 68`, `Community 10`, `Community 46`, `Community 48`, `Community 19`, `Community 28`, `Community 61`, `Community 62`?**
...
- **Are the 37 inferred relationships involving `Neo4jStore` (e.g. with `Neo4jStore` and `FunctionRecord`) actually correct?**
  _`Neo4jStore` has 37 INFERRED edges - model-reasoned connections that need verification._
- **Are the 52 inferred relationships involving `PipelineConfig` (e.g. with `Language` and `Neo4jStore`) actually correct?**
  _`PipelineConfig` has 52 INFERRED edges - model-reasoned connections that need verification._