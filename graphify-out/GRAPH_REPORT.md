...
## Corpus Check
- 78 files · ~30,422 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1085 nodes · 2335 edges · 64 communities (50 shown, 14 thin omitted)
- Extraction: 77% EXTRACTED · 23% INFERRED · 0% AMBIGUOUS · INFERRED: 543 edges (avg confidence: 0.52)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9b8cdc0b`
...
Cohesion: 0.11
Nodes (32): OllamaClient, FunctionRecord, PipelineConfig, One extracted function or method from a source file., Description service for LLM-generated function summaries.  Wraps ``OllamaClient., Embedding service for code and description vectors.  Wraps ``OllamaClient.embed(, QueryMatch, QueryResult (+24 more)
...
Cohesion: 0.16
Nodes (18): datetime, DescriptionService, Generates structured JSON descriptions of functions via OllamaClient., Generates structured JSON descriptions of functions via OllamaClient., EmbeddingService, Generates code and description embeddings using OllamaClient., Generates code and description embeddings using OllamaClient., FunctionExtractor (+10 more)
...
Cohesion: 0.09
Nodes (27): bool, diff_generator_node(), Compute a unified diff between the original and generated code.      Expected st, Static validator node.  Replaces the former reviewer node. Responsibilities are, Validate generated code for structural correctness.      Runs Python syntax vali, static_validator_node(), Shared helpers for graph node implementations., Return a required value from `state` or raise ValueError. (+19 more)
...
Cohesion: 0.10
Nodes (33): branch_creator_node(), Branch creator node.  Creates (or checks out) a task branch in the target reposi, Create a task branch in the target repository.      Reads ``repo_path`` and ``ta, Git committer node.  Stages the modified target file and creates a git commit on, build_ast_graph(), graphify_indexer_node(), Graphify indexer — internal graph-building utility.  Provides `build_ast_graph`,, Run AST-only graphify extraction and write graph.json to graph_dir. (+25 more)
...
Cohesion: 0.13
Nodes (26): ndarray, A SIMILAR_TO relationship between two Function nodes., A SIMILAR_TO relationship between two Function nodes., A SIMILAR_TO relationship between two Function nodes., SimilarityConfig, SimilarityEdge, Neo4jStore, Neo4j data store for Function nodes and SIMILAR_TO relationships.  Uses the offi (+18 more)
...
Cohesion: 0.09
Nodes (20): LiteralString, _ddl(), Idempotently create constraints and indexes.          Args:             vector_d, Async Neo4j driver wrapper for Function nodes and SIMILAR_TO edges., Idempotently create constraints and indexes.          Args:             vector_d, Idempotently create constraints and indexes.          Args:             vector_d, Mark functions not in ``seen_ids`` as deleted. Returns count., Format a DDL template with integer-only substitutions and assert LiteralString. (+12 more)
...
Cohesion: 0.13
Nodes (32): Neo4jStore, Neo4jConfig, ReporterConfig, _build_report(), _combined_sim(), _compute_clusters(), _compute_cohesion_scores(), _cosine() (+24 more)
...
- **Why does `GraphState` connect `Community 30` to `Community 33`, `Community 39`, `Community 9`, `Community 12`, `Community 45`, `Community 19`, `Community 54`, `Community 26`, `Community 59`, `Community 28`, `Community 29`, `Community 31`?**
  _High betweenness centrality (0.247) - this node is a cross-community bridge._
- **Why does `OllamaClient` connect `Community 2` to `Community 32`, `Community 4`, `LLM Pipeline & Sandbox`, `Community 10`, `Community 49`, `Community 19`, `Community 57`?**
  _High betweenness centrality (0.233) - this node is a cross-community bridge._
- **Why does `datetime` connect `Community 10` to `Test Infrastructure & Fixtures`, `Community 48`, `Community 54`, `Community 28`, `Community 62`, `Community 31`?**
