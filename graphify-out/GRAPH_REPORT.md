...
## Corpus Check
- 89 files · ~31,310 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1075 nodes · 2360 edges · 77 communities (61 shown, 16 thin omitted)
- Extraction: 78% EXTRACTED · 22% INFERRED · 0% AMBIGUOUS · INFERRED: 528 edges (avg confidence: 0.52)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `64ccda01`
...
Cohesion: 0.16
Nodes (23): OllamaClient, Neo4jStore, Async Neo4j driver wrapper for Function nodes and SIMILAR_TO edges., Close the driver connection pool., PipelineConfig, QueryMatch, QueryResult, Semantic search engine for functions stored in Neo4j.  Embeds a free-text query (+15 more)
...
Cohesion: 0.06
Nodes (46): ContextAssembler, Deterministic, capped context assembler that consumes a snapshot.      Returns a, ContextBudget, Allocates ranked files against char and file-count limits.      Limits are adv, Allocates ranked files against char-per-file, file-count, and token limits., GraphQuery, Graph query utilities for the retrieval pipeline.  GraphQuery loads graph.json o, Map resolved absolute file paths to their highest node score.          Nodes wit (+38 more)
...
Cohesion: 0.27
Nodes (6): ReportTask, Routes Task instances to the appropriate handler.      Resources (OllamaClient,, Routes Task instances to the appropriate handler.      Resources (OllamaClient,, Routes Task instances to the appropriate handler.      Resources (OllamaClient,, TaskDispatcher, PipelineConfig

### Community 41 - "Community 41"
Cohesion: 0.15
Nodes (10): Return ``{function_id: source_hash}`` for all live functions in the repo., Mark functions not in ``seen_ids`` as deleted. Returns count., Return ``[(id, code_embedding, description_embedding)]`` for all live functions., Return top-N code-similar functions using the HNSW vector index.          Result, Return top-N description-similar functions using the HNSW vector index., Delete all SIMILAR_TO edges originating from functions in this repo.          Ca, Return qualifiedName, filePath, and description for each requested function id., bool (+2 more)
...
Cohesion: 0.08
Nodes (31): _build_pipeline_blocks(), _build_report_blocks(), notify_pipeline_result(), notify_report_result(), notify_scheduled_run(), Fire-and-forget Slack notification for pipeline completion events., Build a Slack Block Kit block list from a PipelineResult., Post a pipeline completion or failure notice to the configured Slack channel. (+23 more)
...
Cohesion: 0.21
Nodes (15): _extract_from_file(), _is_test_file(), Return one FunctionRecord per function/method found in the repository., Return True if the relative path matches any configured test pattern., _record_id(), _source_hash(), FunctionRecord, One extracted function or method from a source file. (+7 more)
...
Cohesion: 0.18
Nodes (9): _ddl(), Idempotently create constraints and indexes.          Args:             vector_d, Create vector indexes once the embedding dimension is known., Format a DDL template with integer-only substitutions and assert LiteralString., Batch-upsert Function nodes using UNWIND for efficiency., LiteralString, FunctionRecord, int (+1 more)
...
Cohesion: 0.08
Nodes (60): Neo4jConfig, ReporterConfig, ReporterConfig, _combined_sim(), _compute_clusters(), _compute_cohesion_scores(), _cosine(), _find_previous_report() (+52 more)
...
Cohesion: 0.22
Nodes (15): Function extractor.  Uses tree-sitter directly to walk the AST of each source fi, _find_functions(), _get_class_name(), _get_name_token(), _get_ts_language(), _get_tsx_language(), Tree-sitter language setup and AST traversal helpers.  Provides lazily-loaded la, Return the text of the first name-bearing child of *node*.      Covers Python id (+7 more)
...
Cohesion: 0.40
Nodes (5): _format_query_result(), Build a Slack mrkdwn text payload for a list of QueryMatch results., Build a Slack mrkdwn text payload for a list of QueryMatch results., Build a Slack mrkdwn text payload for a list of QueryMatch results., str
...
- **Why does `GraphState` connect `Community 30` to `Community 33`, `Community 66`, `Repository Indexer Protocol`, `Community 39`, `Community 9`, `Community 44`, `Community 28`, `Community 45`, `Community 29`, `Community 19`, `Community 26`, `Community 59`, `Community 60`, `Community 61`, `Community 31`?**
  _High betweenness centrality (0.260) - this node is a cross-community bridge._
- **Why does `OllamaClient` connect `Community 2` to `Community 32`, `Community 4`, `LLM Pipeline & Sandbox`, `Community 40`, `Community 73`, `Community 10`, `Community 49`, `Community 19`, `Community 57`, `Community 59`?**
  _High betweenness centrality (0.224) - this node is a cross-community bridge._
- **Why does `datetime` connect `Community 28` to `Test Infrastructure & Fixtures`, `Community 10`, `Community 48`, `Community 60`, `Community 62`?**
