...
6. **embed_code** — sends each changed function's source code to the Ollama embedding model. Unchanged functions skip this stage. Functions whose source code is at or above `context_overflow_char_threshold` characters (default 10 000) are split into overlapping 6 000-character chunks; each chunk is embedded independently and the element-wise mean of the resulting vectors is stored as the function's embedding (`codeEmbeddingStatus: "chunked"`). Functions below the threshold are sent as a single request (truncated to `max_code_chars` as a safety cap).
...
| `codeEmbedding` | list\<float\> \| null | Embedding vector of source code. Large functions (≥ `context_overflow_char_threshold` chars) are embedded via mean-pooled chunk averaging rather than a single pass. Null only if embedding failed entirely. Functions with a null `codeEmbedding` are still included in similarity computation if `descriptionEmbedding` is present. |
